...
pub struct Stake<'info> {
    ...
    /// CHECK: This is the ID of MPL Core Program
...
pub fn handler(ctx: Context<Stake>) -> Result<()> {
    ...
    // Loop to all attributes and save only the ones that are not Staking attributes ("staked" and "stake_at")
    // If we find the "staked" attribute already present, we need to make sure the asset is not already staked, if it is, we return an error
    if let Some(attributes) = &attributes_fetched {
        for attribute in &attributes.attribute_list {
            if attribute.key == "staked" {
                require!(attribute.value == "false", ErrorCode::AlreadyStaked);
        }
        else if attribute.key != "stake_at" {
    ...
    attributes_list.push(Attribute {
        key: "stake_at".to_string(),
        value: Clock::get()?.unix_timestamp.to_string(),
    });

    // Now that we have the complete list of Attributes we either add the Plugin or Update the existing one
    // The Attributes Plugin is an Authority-Managed Plugin, so it needs to be signed by the update authority (PDA of update_authority)
    ...
    // If the Attributes Plugin exist, we update it
    else  {
    ...
    // Note that the FreezeDelegate is an Owner-Managed Plugin, so it needs to be signed by the owner
        AddPluginV1CpiBuilder::new(&ctx.accounts.mpl_core_program.to_account_info())
        .asset(&ctx.accounts.asset.to_account_info())
        .collection(Some(&ctx.accounts.collection.to_account_info()))
        .payer(&ctx.accounts.owner.to_account_info())
